/*
 * gemma.c - Gemma causal LLM decoder
 *
...
#define GEMMA_MAX_DEC_LAYERS 28
...
typedef struct {
    ...
    float    *norm;
    gemma_dec_layer_t layers[GEMMA_MAX_DEC_LAYERS];
} gemma_decoder_t;
...
typedef struct {
    gemma_config_t   config;
    gemma_decoder_t  decoder;
...
int gemma_decoder_load(gemma_decoder_t *dec, multi_safetensors_t *ms,
                       const gemma_config_t *cfg) {
    char name[512];
    ...
    for (int i = 0; i < cfg->dec_layers; i++) {
        gemma_dec_layer_t *l = &dec->layers[i];
...
static int kv_cache_init(gemma_ctx_t *ctx, int max_seq) {
...
static int kv_cache_grow(gemma_ctx_t *ctx, int required) {
...
static float *kv_cache_k_at(gemma_ctx_t *ctx, int layer, int pos) {
    int kv_dim = ctx->config.dec_kv_heads * ctx->config.dec_head_dim;
    return ctx->kv_cache_k + ((size_t)layer * ctx->kv_cache_max + pos) * kv_dim;
}

static float *kv_cache_v_at(gemma_ctx_t *ctx, int layer, int pos) {
    int kv_dim = ctx->config.dec_kv_heads * ctx->config.dec_head_dim;
    return ctx->kv_cache_v + ((size_t)layer * ctx->kv_cache_max + pos) * kv_dim;
}

static int ensure_prefill_buffers(gemma_ctx_t *ctx, int seq_len) {
    const gemma_config_t *cfg = &ctx->config;
...
static int ensure_rope_inv_freq(gemma_ctx_t *ctx, int head_dim, float theta) {
    int half = head_dim / 2;
...
static int ensure_rope_cache(gemma_ctx_t *ctx, int required_pos, int head_dim, float theta) {
...
void gemma_decoder_prefill(gemma_ctx_t *ctx, const float *input_embeds, int seq_len) {
    gemma_decoder_t *dec = &ctx->decoder;
    const gemma_config_t *cfg = &ctx->config;
    int dim = cfg->dec_hidden;
    ...
    for (int layer = 0; layer < cfg->dec_layers; layer++) {
        gemma_dec_layer_t *l = &dec->layers[layer];
...
static void ensure_dec_buffers(gemma_ctx_t *ctx) {
    if (ctx->dec_x) return;
    const gemma_config_t *cfg = &ctx->config;
    int dim = cfg->dec_hidden;
    int q_dim = cfg->dec_heads * cfg->dec_head_dim;
...
int gemma_decoder_forward(gemma_ctx_t *ctx, const float *input_embed) {
    gemma_decoder_t *dec = &ctx->decoder;
    const gemma_config_t *cfg = &ctx->config;
    int dim = cfg->dec_hidden;
    int n_heads = cfg->dec_heads;
