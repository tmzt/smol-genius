...
int main(int argc, char **argv) {
    ...
    if (enc_window_sec >= 0) {
        ...
        ctx->enc_config.enc_n_window_infer = window_frames;
    }
    ...
    if (stream_mode && use_stdin) {
        ...
        qwen_live_audio_t *live = smol_live_audio_start_stdin();
        if (live) {
            text = qwen_transcribe_stream_live(ctx, live);
            smol_live_audio_free(live);
        }
    } else if (stream_mode) {
        /* File-based streaming: load audio fully, then stream-transcribe */
        int ns = 0;
        float *samps = smol_load_wav(input_wav, &ns);
        if (samps) {
