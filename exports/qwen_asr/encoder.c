/*
 * encoder.c - Qwen3-ASR audio encoder forward pass
 *
 * Architecture:
...
/* ========================================================================
 * Conv Stem (split API for GPU encoder)
 * ======================================================================== */
...
float *qwen_asr_encoder_conv_stem(qwen_asr_encoder_t *enc,
    ...
    for (int c = 0; c < n_chunks; c++) {
        ...
        int h1 = (128 + 2 - 3) / 2 + 1;
        int w1 = (chunk_w + 2 - 3) / 2 + 1;
        float *c1 = (float *)malloc(QWEN_ASR_CONV_HIDDEN * h1 * w1 * sizeof(float));
        smol_conv2d(c1, chunk_mel, enc->conv1_weight, enc->conv1_bias,
                     1, QWEN_ASR_CONV_HIDDEN, 128, chunk_w, 3, 3, 2, 1);
        smol_gelu(c1, QWEN_ASR_CONV_HIDDEN * h1 * w1);
        free(chunk_mel);

        int h2 = (h1 + 2 - 3) / 2 + 1;
        int w2 = (w1 + 2 - 3) / 2 + 1;
        float *c2 = (float *)malloc(QWEN_ASR_CONV_HIDDEN * h2 * w2 * sizeof(float));
        smol_conv2d(c2, c1, enc->conv2_weight, enc->conv2_bias,
                     QWEN_ASR_CONV_HIDDEN, QWEN_ASR_CONV_HIDDEN, h1, w1, 3, 3, 2, 1);
        smol_gelu(c2, QWEN_ASR_CONV_HIDDEN * h2 * w2);
        free(c1);

        int h3 = (h2 + 2 - 3) / 2 + 1;
        int w3 = (w2 + 2 - 3) / 2 + 1;
        float *c3 = (float *)malloc(QWEN_ASR_CONV_HIDDEN * h3 * w3 * sizeof(float));
        smol_conv2d(c3, c2, enc->conv3_weight, enc->conv3_bias,
                     QWEN_ASR_CONV_HIDDEN, QWEN_ASR_CONV_HIDDEN, h2, w2, 3, 3, 2, 1);
        smol_gelu(c3, QWEN_ASR_CONV_HIDDEN * h3 * w3);
        free(c2);

        int conv_proj_dim = QWEN_ASR_CONV_HIDDEN * h3;
        float *reshaped = (float *)malloc(w3 * conv_proj_dim * sizeof(float));
        for (int t = 0; t < w3; t++) {
            for (int ch = 0; ch < QWEN_ASR_CONV_HIDDEN; ch++) {
                for (int f = 0; f < h3; f++) {
        ...
        smol_linear_nobias(projected, reshaped, enc->conv_out_weight,
                            w3, conv_proj_dim, d_model);
        free(reshaped);

        float *pe = (float *)malloc(w3 * d_model * sizeof(float));
        smol_sinusoidal_pe(pe, w3, d_model);
        smol_add_inplace(projected, pe, w3 * d_model);
        free(pe);
    ...
    *out_window_starts = window_starts;
    *out_n_windows = n_windows;
    return x;
}

/* ========================================================================
 * Forward Pass (full — conv stem + transformer + output projection)
 * ======================================================================== */
...
float *qwen_asr_encoder_forward(qwen_asr_encoder_t *enc,
    ...
    for (int c = 0; c < n_chunks; c++) {
        ...
        int conv_proj_dim = QWEN_ASR_CONV_HIDDEN * h3; /* 480 * 16 = 7680 */
        float *reshaped = (float *)malloc(w3 * conv_proj_dim * sizeof(float));
        for (int t = 0; t < w3; t++) {
            for (int ch = 0; ch < QWEN_ASR_CONV_HIDDEN; ch++) {
                for (int f = 0; f < h3; f++) {
    ...
    for (int layer = 0; layer < cfg->enc_layers; layer++) {
        qwen_asr_enc_layer_t *l = &enc->layers[layer];

        /* ---- Self-attention ---- */
        smol_layer_norm(x_norm, x, l->attn_norm_weight, l->attn_norm_bias,
        ...
        smol_linear(proj_out, attn_out, l->wo_weight, l->wo_bias,
                     total_tokens, d_model, d_model);
        smol_add_inplace(x, proj_out, total_tokens * d_model);

        /* ---- FFN ---- */
        smol_layer_norm(x_norm, x, l->ffn_norm_weight, l->ffn_norm_bias,
                        total_tokens, d_model, 1e-5f);

        /* GELU FFN: fc1 -> GELU -> fc2 */
        smol_linear(ffn_mid, x_norm, l->fc1_weight, l->fc1_bias,
                     total_tokens, d_model, ffn_dim);
        smol_gelu(ffn_mid, total_tokens * ffn_dim);
        smol_linear(ffn_out, ffn_mid, l->fc2_weight, l->fc2_bias,
                     total_tokens, ffn_dim, d_model);
        smol_add_inplace(x, ffn_out, total_tokens * d_model);

    }

    /* Final LayerNorm */
    smol_layer_norm(x, x, enc->ln_post_weight, enc->ln_post_bias,
                    total_tokens, d_model, 1e-5f);

    /* Projection: proj1 (GELU) -> proj2 */
    float *proj_mid = (float *)malloc(total_tokens * d_model * sizeof(float));
    smol_linear(proj_mid, x, enc->proj1_weight, enc->proj1_bias,
                 total_tokens, d_model, d_model);
    smol_gelu(proj_mid, total_tokens * d_model);

    float *enc_output = (float *)malloc(total_tokens * output_dim * sizeof(float));
    smol_linear(enc_output, proj_mid, enc->proj2_weight, enc->proj2_bias,
