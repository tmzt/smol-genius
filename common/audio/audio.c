/*
 * audio.c - WAV loading and mel spectrogram computation
 *
...
float *smol_parse_wav_buffer(const uint8_t *data, size_t file_size, int *out_n_samples) {
...
float *smol_load_wav(const char *path, int *out_n_samples) {
    ...
    if (!f) {
        fprintf(stderr, "smol_load_wav: cannot open %s\n", path);
        return NULL;
    }
    fseek(f, 0, SEEK_END);
    ...
    fclose(f);
    float *samples = smol_parse_wav_buffer(data, (size_t)file_size, out_n_samples);
    free(data);
    return samples;
}
...
float *smol_read_pcm_stdin(int *out_n_samples) {
    ...
    if (size < 4) {
        fprintf(stderr, "smol_read_pcm_stdin: no data on stdin\n");
    ...
    if (memcmp(buf, "RIFF", 4) == 0) {
        ...
        float *samples = smol_parse_wav_buffer(buf, size, out_n_samples);
        free(buf);
        return samples;
    }
...
float *smol_mel_spectrogram(const float *samples, int n_samples, int *out_frames) {
    ...
    if (n_frames <= 0) {
        fprintf(stderr, "smol_mel_spectrogram: audio too short (%d samples)\n", n_samples);
        free(padded);
        return NULL;
    }
...
static void live_audio_append(smol_live_audio_t *la, const float *data, int n_new) {
...
static void live_audio_convert_and_append(smol_live_audio_t *la,
...
smol_live_audio_t *smol_live_audio_start_stdin(void) {
    ...
    if (hdr_read < 4) {
        fprintf(stderr, "smol_live_audio_start_stdin: no data on stdin\n");
        return NULL;
    }
    ...
    if (hdr_read >= 44 && memcmp(header, "RIFF", 4) == 0 && memcmp(header + 8, "WAVE", 4) == 0) {
        ...
        if (wav_format != 1 || wav_bits != 16 || wav_channels < 1) {
            fprintf(stderr, "smol_live_audio_start_stdin: unsupported WAV format "
                    "(need 16-bit PCM, got fmt=%d bits=%d)\n", wav_format, wav_bits);
            return NULL;
        }
        if (wav_sample_rate != SAMPLE_RATE) {
            fprintf(stderr, "smol_live_audio_start_stdin: WAV sample rate is %d Hz, "
        ...
        if (wav_channels != 1) {
            fprintf(stderr, "smol_live_audio_start_stdin: WAV has %d channels, "
        ...
        if (data_chunk_offset == 0) {
            fprintf(stderr, "smol_live_audio_start_stdin: WAV data chunk not found in header\n");
            return NULL;
        }
        if (smol_verbose >= 2)
    ...
    smol_live_audio_t *la = (smol_live_audio_t *)calloc(1, sizeof(smol_live_audio_t));
    if (!la) return NULL;
    ...
    if (pthread_create(&la->thread, NULL, live_reader_thread, rctx) != 0) {
        fprintf(stderr, "smol_live_audio_start_stdin: failed to create reader thread\n");
        free(rctx);
        smol_live_audio_free(la);
        return NULL;
    }
