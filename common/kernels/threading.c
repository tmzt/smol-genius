/*
 * threading.c - Thread pool for smol-genius inference kernels
...
static struct {
    pthread_t threads[SMOL_MAX_THREADS - 1];
    int tids[SMOL_MAX_THREADS - 1];
    int n_threads;
    int shutdown;
...
void smol_set_threads(int n) {
    if (n < 1) n = 1;
    if (n > SMOL_MAX_THREADS) n = SMOL_MAX_THREADS;
