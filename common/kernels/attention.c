...
static void smol_swa_heads(float *out, const float *Q, const float *K,
                            const float *V, int seq_q, int seq_k,
                            int n_heads, int n_kv_heads, int head_dim,
...
static void swa_worker(int tid, int n_threads, void *arg) {
    swa_task_t *t = (swa_task_t *)arg;
    int chunk = (t->n_heads + n_threads - 1) / n_threads;
    int h0 = tid * chunk;
    int h1 = h0 + chunk;
    if (h1 > t->n_heads) h1 = t->n_heads;
    ...
    smol_swa_heads(t->out, t->Q, t->K, t->V,
                    t->seq_q, t->seq_k, t->n_heads, t->n_kv_heads,
                    t->head_dim, t->scale, t->q_offset, t->window_size, h0, h1);
}
...
void smol_sliding_window_attention(float *out, const float *Q, const float *K,
    ...
    if (smol_get_thread_count() > 1 && n_heads >= 2 && (seq_q >= 2 || seq_k >= 128)) {
        ...
        smol_parallel_for(swa_worker, &task);
        return;
    }

    smol_swa_heads(out, Q, K, V,
                    seq_q, seq_k, n_heads, n_kv_heads,
                    head_dim, scale, q_offset, window_size, 0, n_heads);
}

/* ========================================================================
 * Softcapped Attention Variants (Gemma 2)
 *
 * score = cap * tanhf(score / cap)  applied unconditionally after dot*scale
 * ======================================================================== */

static void smol_causal_attention_softcap_heads(
        float *out, const float *Q, const float *K, const float *V,
        int seq_q, int seq_k, int n_heads, int n_kv_heads, int head_dim,
        float scale, int q_offset, float cap, int head_start, int head_end) {
    int heads_per_kv = n_heads / n_kv_heads;
    int q_hidden = n_heads * head_dim;
    int kv_hidden = n_kv_heads * head_dim;
...
typedef struct {
    float *out;
    const float *Q, *K, *V;
    int seq_q, seq_k, n_heads, n_kv_heads, head_dim;
    float scale, cap;
    int q_offset;
} causal_softcap_task_t;

static void causal_softcap_worker(int tid, int n_threads, void *arg) {
    causal_softcap_task_t *t = (causal_softcap_task_t *)arg;
    int chunk = (t->n_heads + n_threads - 1) / n_threads;
    int h0 = tid * chunk;
    int h1 = h0 + chunk;
    if (h1 > t->n_heads) h1 = t->n_heads;
    if (h0 >= h1) return;
    smol_causal_attention_softcap_heads(t->out, t->Q, t->K, t->V,
        t->seq_q, t->seq_k, t->n_heads, t->n_kv_heads,
        t->head_dim, t->scale, t->q_offset, t->cap, h0, h1);
}
...
void smol_causal_attention_softcap(float *out, const float *Q, const float *K,
    ...
    if (smol_get_thread_count() > 1 && n_heads >= 2 && (seq_q >= 2 || seq_k >= 128)) {
        causal_softcap_task_t task = {
            .out = out, .Q = Q, .K = K, .V = V,
            .seq_q = seq_q, .seq_k = seq_k,
            .n_heads = n_heads, .n_kv_heads = n_kv_heads,
            .head_dim = head_dim, .scale = scale, .cap = cap,
            .q_offset = q_offset
        };
        smol_parallel_for(causal_softcap_worker, &task);
        return;
    }
    smol_causal_attention_softcap_heads(out, Q, K, V,
        seq_q, seq_k, n_heads, n_kv_heads,
        head_dim, scale, q_offset, cap, 0, n_heads);
}

static void smol_swa_softcap_heads(
        float *out, const float *Q, const float *K, const float *V,
        int seq_q, int seq_k, int n_heads, int n_kv_heads, int head_dim,
...
typedef struct {
    ...
    int seq_q, seq_k, n_heads, n_kv_heads, head_dim;
    float scale, cap;
    int q_offset, window_size;
} swa_softcap_task_t;

static void swa_softcap_worker(int tid, int n_threads, void *arg) {
    swa_softcap_task_t *t = (swa_softcap_task_t *)arg;
    int chunk = (t->n_heads + n_threads - 1) / n_threads;
    int h0 = tid * chunk;
    int h1 = h0 + chunk;
    if (h1 > t->n_heads) h1 = t->n_heads;
    if (h0 >= h1) return;
    smol_swa_softcap_heads(t->out, t->Q, t->K, t->V,
        t->seq_q, t->seq_k, t->n_heads, t->n_kv_heads,
        t->head_dim, t->scale, t->q_offset, t->window_size, t->cap, h0, h1);
}
...
void smol_sliding_window_attention_softcap(float *out, const float *Q, const float *K,
                                            ...
                                            float cap) {
    if (smol_get_thread_count() > 1 && n_heads >= 2 && (seq_q >= 2 || seq_k >= 128)) {
        swa_softcap_task_t task = {
            .out = out, .Q = Q, .K = K, .V = V,
            .seq_q = seq_q, .seq_k = seq_k,
            .n_heads = n_heads, .n_kv_heads = n_kv_heads,
            .head_dim = head_dim, .scale = scale, .cap = cap,
            .q_offset = q_offset, .window_size = window_size
        };
        smol_parallel_for(swa_softcap_worker, &task);
        return;
    }
    smol_swa_softcap_heads(out, Q, K, V,
        seq_q, seq_k, n_heads, n_kv_heads,
        head_dim, scale, q_offset, window_size, cap, 0, n_heads);
}
