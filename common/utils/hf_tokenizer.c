/*
 * hf_tokenizer.c - GPT-2 BPE tokenizer for HuggingFace
 *
...
static int merge_rank(const hf_tokenizer_t *tok, const char *a, const char *b) {
...
static int encode_bpe_word(const hf_tokenizer_t *tok, const char *mapped,
...
static int load_tokenizer_json(hf_tokenizer_t *tok, const char *path) {
    FILE *f = fopen(path, "rb");
    if (!f) {
        fprintf(stderr, "hf_tokenizer: cannot open %s\n", path);
        return -1;
    }
    ...
    if (find_json_key(&p, "model") != 0) {
        fprintf(stderr, "hf_tokenizer: 'model' key not found\n");
        free(json);
        return -1;
    }
    ...
    if (find_json_key(&p, "vocab") != 0) {
        fprintf(stderr, "hf_tokenizer: 'vocab' key not found in model\n");
        free(json);
        return -1;
    }
    ...
    if (find_json_key(&p, "merges") != 0) {
        fprintf(stderr, "hf_tokenizer: 'merges' key not found in model\n");
        free(json);
        return -1;
    }
...
hf_tokenizer_t *hf_tokenizer_load(const char *model_dir) {
    ...
    hf_tokenizer_t *tok = (hf_tokenizer_t *)calloc(1, sizeof(*tok));
    if (!tok) return NULL;

    if (load_tokenizer_json(tok, path) != 0) {
        hf_tokenizer_free(tok);
        return NULL;
    }
