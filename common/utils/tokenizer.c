/*
 * tokenizer.c - smol BPE tokenizer (GPT-2 byte-level)
 *
 * Supports:
...
static int merge_rank(const smol_tokenizer_t *tok, const char *a, const char *b) {
...
static int encode_bpe_word(const smol_tokenizer_t *tok, const char *mapped, int **out_ids, int *out_n) {
    *out_ids = NULL;
    *out_n = 0;
...
static int load_merges_map(smol_tokenizer_t *tok, const char *merges_path) {
...
smol_tokenizer_t *smol_tokenizer_load(const char *vocab_json_path) {
    ...
    if (!f) {
        fprintf(stderr, "smol_tokenizer_load: cannot open %s\n", vocab_json_path);
        return NULL;
    }
    ...
    smol_tokenizer_t *tok = (smol_tokenizer_t *)calloc(1, sizeof(smol_tokenizer_t));
    if (!tok) {
        free(json);
        return NULL;
    }
    ...
    if (derive_merges_path(vocab_json_path, merges_path, sizeof(merges_path)) == 0) {
        if (load_merges_map(tok, merges_path) != 0 && smol_verbose >= 2) {
